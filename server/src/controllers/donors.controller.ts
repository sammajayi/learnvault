...
export const getDonorImpact = async (
	...
	try {
		...
		const startLedgerRaw = Number(process.env.STARTING_LEDGER ?? "460000000")
		const startLedger = Number.isFinite(startLedgerRaw)
			? startLedgerRaw
			: 460000000
		...
		const scholarToMilestones = new Map<
			string,
		...
		const averageCompletionRate =
			totalMilestones > 0 ? totalMilestonesCompleted / totalMilestones : 0
...
async function fetchScholarMilestones(
	...
	try {
		...
		const reports = await milestoneStore.getReportsForScholar(
			scholarAddress,
			{},
		)
